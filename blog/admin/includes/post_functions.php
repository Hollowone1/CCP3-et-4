...
function getPostAuthorById($user_id)
{
    ...
    $result = $conn->prepare($sql);
    $result->execute();
    $getPost=$result->fetch(PDO::FETCH_ASSOC);
	if ($getPost) {
		// return username
...
function createPost($request_values)
	{
		...
		$body = htmlentities(htmlspecialchars($request_values['body']));
		if (isset($request_values['topic_id'])) {
			$topic_id = htmlspecialchars($request_values['topic_id']);
		}
		if (isset($request_values['publish'])) {
			$published = htmlspecialchars($request_values['publish']);
		}
		...
		if ($post=$result->rowCount() > 0) { // if post exists
		...
		if (count($errors) == 0) {
            ...
            $result->execute();
            if($post=$result->fetch()){ // if post created successfully
	...
	* * * * * * * * * * * * * * * * * * * * * */
	...
	function updatePost($request_values)
	{
		...
		$post_id = htmlspecialchars($request_values['post_id']);
		if (isset($request_values['topic_id'])) {
			$topic_id = htmlspecialchars($request_values['topic_id']);
		}
		// create slug: if title is "The Storm Is Over", return "the-storm-is-over" as slug
		...
		if (count($errors) == 0) {
			...
			if($topic){ // if post created successfully
				if (isset($topic_id)) {
					$inserted_post_id = $conn->lastInsertId();
					// create relationship between post and topic
					$sql = "INSERT INTO post_topic (post_id, topic_id) VALUES($inserted_post_id, $topic_id)";
